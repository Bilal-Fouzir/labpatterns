	...
	public Symptom createSymptom(String symptomName) {
		...
		if (sm!=null) {
			return sm;
			    ...
			    List<Double> index1 = Arrays.asList(5.0, 4.8, 3.7, 0.9, 0.8, 5.5);
			    ...
			    if (impact!=0)  {
			    	if (digestiveSymptom.contains(symptomName)) {
			    		sm=new DigestiveSymptom(symptomName,(int)index, impact);
			    		sortuak.add(sm);
			    		return sm;
			    	}
			    	if (neuroMuscularSymptom.contains(symptomName)) {
				    		sm=new NeuroMuscularSymptom(symptomName,(int)index, impact);
				    		sortuak.add(sm);
				    		return sm;
				    	}
			    		
			    	if (respiratorySymptom.contains(symptomName)){
			    		sm=new RespiratorySymptom(symptomName,(int)index, impact);
			    		sortuak.add(sm);
			    		return sm;
			    	};
			    }
